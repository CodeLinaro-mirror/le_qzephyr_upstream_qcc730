...
static uart_status uart_xfr_init(uart_xfr *xfr, uint8_t *buf, uint32_t size, uint32_t timeout, uart_dir dir, uint32_t thres)
{
	ARG_UNUSED(timeout);
	ARG_UNUSED(dir);
...
static void uart_qcc730_irq_config_func(const struct device *dev)
{
	...
	IRQ_CONNECT(DT_INST_IRQN(0),
		...
		uart_qcc730_isr, DEVICE_DT_INST_GET(0),
		0);
...
DEVICE_DT_INST_DEFINE(n,                                         \
			  uart_qcc730_init,                                           \
			  NULL,                                                       \
			  &uart_qcc730_dev_data_##n,                                  \
			  &uart_qcc730_dev_cfg_##n,                                   \
			  PRE_KERNEL_1,                                               \
			  CONFIG_SERIAL_INIT_PRIORITY,                                \
			  &uart_qcc730_driver_api);			
